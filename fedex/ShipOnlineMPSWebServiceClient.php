...
// if(is_object($response)){
	...
	if($response->RequestedShipment->MasterTrackingId && $response->RequestedShipment->PackageCount>1){
		$a = $response->RequestedShipment->MasterTrackingId;
		$b = 2;
		$c = $response->RequestedShipment->PackageCount;
		for($k=$b;$k>=$b&&$k<=$c;$k++){
			...
			$trackid = $response{'RequestedShipment'}{'MasterTrackingId'}{'TrackingIdType'};
			$tracknum = $response{'RequestedShipment'}{'MasterTrackingId'}{'TrackingNumber'};
			...
			function addPackageLineItem14($value){
