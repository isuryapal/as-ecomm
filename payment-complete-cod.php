<?php
	...
	if(isset($_GET['txnId']) && !empty($_GET['txnId'])){
		...
		include_once 'fedex/ShipWebServiceClient.php';
		...
		if($result){
			...
			if($purchaseDetails){
				...
				header("location: ".BASE_URL."/chekout-order-summary.php?error=TRANSACTIONPAYMENTFAILED");
				exit;
			}
		} else {
			header("location: ".BASE_URL."/chekout-order-summary.php?error=TRANSACTIONFAILED");
			exit;
		}
	} else {
		header("location: ".BASE_URL."/chekout-order-summary.php");
		exit;
	}
?>
<!DOCTYPE>
