<?php
	...
	if(isset($_GET['page']) && !empty($_GET['page'])){
		$parentPageURL = 'discount-coupon-master.php?page='.$_GET['page'];
	}else{
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			...
			//$category = trim($admin->escape_string($admin->strip_all($_POST['category'])));
			
			if(empty($name)){
				if(isset($_GET['page']) && !empty($_GET['page'])){
					header("location:".$pageURL."?registerfail&page=".$_GET['page']."&msg=Please enter a coupon code");
				}else{
					header("location:".$pageURL."?registerfail&msg=Please enter a coupon code");	
				}
				exit();
			}/* elseif(count($_POST['category'])==0){
				...
				if(isset($_GET['page']) && !empty($_GET['page'])){
					header("location:".$pageURL."?registersuccess&page=".$_GET['page']);
				}else{
	...
	if(isset($_POST['id']) && !empty($_POST['id'])) {
		if($csrf->check_valid('post')) {
			...
			if(empty($id) || empty($name)){
				if(isset($_GET['page']) && !empty($_GET['page'])){
					header("location:".$pageURL."?updatefail&msg=Please enter a coupon code&edit&id=".$id."&page=".$_GET['page']);
				}else{
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<div class="form-group">
							<div class="row">
								<div class="col-sm-3">
									...
									<input type="hidden" name="edited_by" value="<?php echo $loggedInUserDetailsArr['id']; ?>"/>
								...
								<div class="col-sm-3">
									<label>Coupon Value<em>*</em></label>
						...
						<div class="form-group">
							<div class="row">
								<div class="col-sm-3">
									<label>Coupon To Apply<em>*</em></label>
									<select name="coupon_apply[]" class="select-multiple" multiple placeholder="Select To Apply On">
										<option value="b2b" 
										<?php
										if(isset($_GET['edit'])){
											$coupon_apply = explode(",", $data['coupon_apply']);
											if(in_array("b2b", $coupon_apply)){ echo "selected"; }
										}
										?>
										>
										B2B
										...
										if(isset($_GET['edit'])){
											$coupon_apply = explode(",", $data['coupon_apply']);
											if(in_array("b2c", $coupon_apply)){ echo "selected"; }
										}
										?>
										>
										B2C
