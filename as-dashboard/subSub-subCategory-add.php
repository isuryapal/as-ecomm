<?php
	...
	$pageName = "Sub SubCategory Master";
	$parentPageURL = 'subSub-subCategory-master.php?subsubcate_id='.$subsubcate_id.'&subcate_id='.$category_id.'&category_id='.$category_ids;
	$pageURL = 'subSub-subCategory-add.php';
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			...
			if(empty($name)){
				header("location:".$pageURL."?registerfail&msg=Please enter a name&subsubcate_id=".$subsubcate_id."&subcate_id=".$category_id."&category_id=".$category_ids);
				exit();
			}
			/* else if(!in_array($_FILES['image_name']['type'],$allowed_ext)) {
				...
				//add to database
				$result = $admin->addSubSubSubCategory($_POST,$_FILES);
				header("location:".$pageURL."?registersuccess&subsubcate_id=".$subsubcate_id."&subcate_id=".$category_id."&category_id=".$category_ids);
			}
		}
	}
	if(isset($_GET['edit'])){
		$id = $admin->escape_string($admin->strip_all($_GET['id']));
		$data = $admin->getuniqueSubsubsubCategory($id);
	...
	if(isset($_POST['update'])) {
		if($csrf->check_valid('post')) {
			...
			if(empty($id) || empty($name)){
				header("location:".$pageURL."?updatefail&msg=Please enter a name&edit&id=".$id."&subsubcate_id=".$subsubcate_id."&subcate_id=".$category_id."&category_id=".$category_ids);
				exit();
			}
			/* else if(!empty($_FILES['image_name']['name']) and !in_array($_FILES['image_name']['type'],$allowed_ext)) {
				...
				//update to database
				$result = $admin->updateSubSubSubCategory($_POST,$_FILES);
				header("location:".$pageURL."?updatesuccess&edit&id=$id&subsubcate_id=".$subsubcate_id."&subcate_id=".$category_id."&category_id=".$category_ids);
			}
		}
	}
?>
...
<?php	} ?>
			<form role="form" action="" method="post" id="form" enctype="multipart/form-data">
						...
						<div class="form-group">
							<div class="row">
								<div class="col-sm-4">
									...
									<input  type="text" class="form-control" required="required" name="name" id="" value="<?php if(isset($_GET['edit'])){ echo $data['subsubsub_name']; }?>"/>
								</div>
				...
				<input type="hidden" name="subsub_category_id" value="<?php echo $subsubcate_id; ?>" />
				<input type="hidden" name="subcatIds" value="<?php if(isset($_GET['subsubcate_id'])){ echo $_GET['subsubcate_id']; }  ?>">
<?php
			if(isset($_GET['edit'])){ ?>
