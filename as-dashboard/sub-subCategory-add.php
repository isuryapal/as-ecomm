<?php
	...
	$category_ids = $admin->escape_string($admin->strip_all($_GET['category_id']));
	$category_id = $admin->escape_string($admin->strip_all($_GET['subcate_id']));

	$pageName = "Sub SubCategory Master";
	$parentPageURL = 'sub-subCategory-master.php?subcate_id='.$category_id.'&category_id='.$category_ids;
	$pageURL = 'sub-subCategory-add.php';
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			...
			if(empty($name)){
				header("location:".$pageURL."?registerfail&msg=Please enter a name&category_id=".$category_ids);
				exit();
			}
			/* else if(!in_array($_FILES['image_name']['type'],$allowed_ext)) {
				...
				//add to database
				$result = $admin->addsubSubCategory($_POST,$_FILES);
				header("location:".$pageURL."?registersuccess&category_id=$category_ids&subcate_id=".$category_id);
			}
		}
	}
	if(isset($_GET['edit'])){
		$id = $admin->escape_string($admin->strip_all($_GET['id']));
		$data = $admin->getUniqueSubSubCategoryByIDs($id);
	...
	if(isset($_POST['update'])) {
		if($csrf->check_valid('post')) {
			...
			if(empty($id) || empty($name)){
				header("location:".$pageURL."?updatefail&msg=Please enter a name&edit&id=".$id."&category_id=".$category_ids);
				exit();
			}
			/* else if(!empty($_FILES['image_name']['name']) and !in_array($_FILES['image_name']['type'],$allowed_ext)) {
				...
				//update to database
				$result = $admin->updateSubSubCategory($_POST,$_FILES);
				header("location:".$pageURL."?updatesuccess&edit&category_id=$category_ids&id=$id&subcate_id=$category_id");
			}
		}
	}
?>
...
<?php	} ?>
			<form role="form" action="" method="post" id="form" enctype="multipart/form-data">
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<div class="form-group">
							<div class="row">
								<div class="col-sm-4">
									<label>Sub SubCategory Name<em>*</em></label>
									<input  type="text" class="form-control" required="required" name="name" id="" value="<?php if(isset($_GET['edit'])){ echo $data['subcategory_name']; }?>"/>
								</div>
							</div>
						</div>
						<div class="form-group">
				...
				<input type="hidden" name="subcatIds" value="<?php if(isset($_GET['subcate_id'])){ echo $_GET['subcate_id']; }  ?>">
<?php
			if(isset($_GET['edit'])){ ?>
