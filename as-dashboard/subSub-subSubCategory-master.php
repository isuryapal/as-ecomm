<?php
	...
	$category_id = $admin->escape_string($admin->strip_all($_GET['subsubsubcate_id']));

	$pageName = "SubSub - SubSub Categories";
	$pageURL = 'subSub-subSubCategory-master.php?subsubsubcate_id'.$_REQUEST['subsubsubcate_id'].'&subsubcate_id='.$_REQUEST['subsubcate_id'].'&subcate_id='.$_REQUEST['subcate_id'].'&category_id='.$_REQUEST['category_id'];
	$addURL = 'subSub-subSubCategory-add.php';
	$deleteURL = 'subsubsubsub-category-delete.php';
	$tableName = 'subsubsubsubCategory';
	...
	$sql = "SELECT * FROM ".PREFIX.$tableName." where subsubsubCate_id='$category_id' order by created DESC";
...
<body class="sidebar-wide">
	...
	<div class="page-container">
		...
		-->
			...
			<a href="subSub-subCategory-master.php?subsubcate_id=<?php echo $_REQUEST['subsubcate_id'] ?>&subcate_id=<?php echo $_REQUEST['subcate_id']; ?>&category_id=<?php echo $_REQUEST['category_id']; ?>" class="label label-primary">Back to SubSub Category Master</a>
			<a href="subSub-subSubCategory-add.php?subsubsubcate_id=<?php echo $_REQUEST['subsubsubcate_id']; ?>&subsubcate_id=<?php echo $_REQUEST['subsubcate_id']; ?>&subcate_id=<?php echo $_REQUEST['subcate_id']; ?>&category_id=<?php echo $_REQUEST['category_id']; ?>" class="label label-primary">Add <?php echo $pageName; ?></a><br/><br/>
	<?php
...
<?php
						...
						// { $gender = "( Angle )"; }else{ $gender = ""; }
							$subsubsubcateDetails = $admin->getuniqueSubsubsubCategory($_REQUEST['subsubsubcate_id']);
?>		
							<tr>
								<td><?php echo $x++; ?></td>
								<td><?php echo $row['subsubsubsub_name']?></td>
								<td><?php echo $subsubsubcateDetails['subsubsub_name']; ?></td>
								<td><?php if($row['active']=='1'){echo 'Yes';}else{echo 'No';} ?></a></td>
								<td>
									<a href="<?php echo $addURL; ?>?edit&id=<?php echo $row['id'] ?>&subsubsubcate_id=<?php echo $_REQUEST['subsubsubcate_id']; ?>&subsubcate_id=<?php echo $_REQUEST['subsubcate_id']; ?>&subcate_id=<?php echo $_REQUEST['subcate_id']; ?>&category_id=<?php echo $_REQUEST['category_id']; ?>" name="edit" class="" title="Click to edit this row"><i class="icon-pencil"></i></a>
									<a class="" href="<?php echo $deleteURL; ?>?id=<?php echo $row['id']; ?>&subsubsubcate_id=<?php echo $_REQUEST['subsubsubcate_id']; ?>&subsubcate_id=<?php echo $_REQUEST['subsubcate_id']; ?>&subcate_id=<?php echo $_REQUEST['subcate_id']; ?>&category_id=<?php echo $_REQUEST['category_id']; ?>" onclick="return confirm('Are you sure you want to delete?');" title="Click to delete this row, this action cannot be undone."><i class="icon-remove3"></i></a>
