	...
	if(isset($_POST['update'])) {
		if($csrf->check_valid('post')) {
			...
			if(empty($return_policy) ){
				header("location:".$pageURL."?updatefail&msg=Please enter return policy details");
				exit();
			}
			else {
				//update to database
				if(empty($id)){
					$sql = "INSERT INTO ".PREFIX."cms_master(return_policy) VALUES ('".$return_policy."')";
					//echo $sql; exit;
					$admin->query($sql);
					header("location:".$pageURL."?updatesuccess");
				}else{
					$sql = "UPDATE ".PREFIX."cms_master SET `return_policy`='".$return_policy."'";
					//echo $sql; exit;
					$admin->query($sql);
			...
			<form role="form" action="" method="post" id="form">
				<div class="panel panel-default">
					<div class="panel-heading">
						<h6 class="panel-title"><i class="icon-library"></i> Return Policy Details</h6>
					</div>
						...
						<div class="form-group">
							<div class="row">
								<div class="col-sm-12">
									...
									<textarea name="return_policy" rows="7" id="return_policy"> <?php  echo $data['return_policy']; ?></textarea>
								</div>
...
<?php 	include "include/footer.php"; ?>
	...
	<script type="text/javascript">
		var editor = CKEDITOR.replace( 'return_policy', {
