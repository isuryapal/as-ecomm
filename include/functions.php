	...
	class Functions extends Database {
		...
		function getCartAmountAndQuantity($cartObj, $loggedInUserDetailsArr){
			...
			if($cartArr){
				...
				$shippingCharges = $this->getShippingCharge($cartObj, $loggedInUserDetailsArr);
		...
		function getCompletedOrdersByCustomerId($customerId){
			...
			$query ="SELECT * FROM ".PREFIX."order WHERE `customer_id`='".$customerId."' and is_deleted='0' and (`payment_status`='Payment Complete' or (`payment_status`='Payment Pending' and payment_mode='COD')) order by id DESC";
			return $this->query($query);
		}
		...
		function getShippingCharge($cartObj, $loggedInUserDetailsArr){
			...
			if(isset($loggedInUserDetailsArr['id'])){
				$upOne = dirname(__DIR__);
				include $upOne.'/fedex/Rate/RateThisWebServiceClient.php';
				return $content;
			}
		// }else{
		// 	return 0;
		...
		function isCouponApplicableForCustomer($couponId, $loggedInUserId, $orderId = 0){
			...
			if($masterCouponRS->num_rows>0){
				...
				$couponCouponDetails = explode(",",$masterCouponDetails['coupon_apply']);
					// print_r($couponCouponDetails);
				if(in_array($loggedInUserId['user_type'], $couponCouponDetails)){
					...
					if($masterCouponDetails['coupon_usage']=="multiple"){ // anyone can use coupon
						...
						if(empty($orderId)){ // coupon code is being applied
							$query = "select * from ".PREFIX."order_discount_coupons where discount_coupon_id='".$couponId."' and customer_id='".$loggedInUserId['id']."'";
						} else { // user is at payment gateway, allow single coupon code for same transaction
							$query = "select * from ".PREFIX."order_discount_coupons where discount_coupon_id='".$couponId."' and customer_id='".$loggedInUserId['id']."' and order_id!='".$orderId."'";
						}
				...
				if($couponDetailsRS->num_rows>0){ // coupon is valid
					...
					// check whether coupon code apply on specific customer type
						...
						if($couponDetails['coupon_usage']=="single"){
							...
							if($this->isCouponApplicableForCustomer($couponDetails['id'], $loggedInUser, $orderId)){
								// coupon is used in past transaction
		...
		function applyCouponCode($couponCode, $loggedInUserDetailsArr){
			...
			if(count($errorArr)>0){
				...
				if(count($couponDetails)>0){
					...
					$isCouponApplicable = $this->isCouponApplicableForCustomer($couponDetails['id'], $loggedInUserDetailsArr, 0);
					if($isCouponApplicable){ // customer has not used this coupon code yet
						...
						return array(
							"response" => true,
							"responseMsg" => "Invalid coupon code",
							"couponCodeMsg" => "Invalid coupon code",
							"error" => "COUPONUSED"
						);
					}
				} else {
		...
		function addProduct($data, $file){
			...
			/*print_r($data); 
			exit;*/
			$vendor_id = $this->escape_string($this->strip_all($data['id']));
			...
			$sql = "INSERT INTO ".PREFIX."product_master(`product_name`, `product_code`, `hsn_code`, `availability`, `main_image`, `image_one`, `image_two`, `image_three`, `image_four`, `video_one`, `video_two`, `video_three`, `video_four`, `video_five`, `price`, `discount_price`, `b2b_price`, `b2b_discount_price`, `b2b_min_qty`, `tax`, `description`, `active`, `meta_title`, `meta_keyword`, `meta_description`, `permalink`, `time`, `feature_product`, is_feature, best_seller, vendor_id, created) VALUES ('".$product_name."','".$product_code."','".$hsn_code."','".$availability."','".$main_image."','".$image_one."','".$image_two."','".$image_three."','".$image_four."','".$video_one."','".$video_two."','".$video_three."','".$video_four."','".$video_five."','".$price."','".$discount_price."','".$b2b_price."','".$b2b_discount_price."','".$b2b_min_qty."','".$tax."','".$description."','".$active."','".$meta_title."','".$meta_keyword."','".$meta_description."','".$permalink."' ,'".$date."','".$feature_product."', '".$is_feature."', '".$best_seller."', '".$vendor_id."', '".$createdDate."')";
			// echo $sql; exit;
			$this->query($sql);
		...
		function AddFedexDetail($cartObj,$id,$idNum){
			$id = $this->escape_string($this->strip_all($id));
			$idNum = $this->escape_string($this->strip_all($idNum));
			$date = date("Y-m-d h:i:s");
			$sql = "INSERT INTO ".PREFIX."fedex (master_id_type, master_num, created) VALUES('".$id."','".$idNum."','".$date."')";
			return $this->query($sql);
		}
